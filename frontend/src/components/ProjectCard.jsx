...
import { PaymentModal, StatusBadge } from './index';
...
const ProjectCard = ({ project, userRole, onProjectUpdate }) => {
  ...
  const handleRequestAccess = async () => {
    ...
    try {
      ...
      addNotification({ type: 'success', title: 'Access Request Sent', message: 'Your request has been sent to the developer.' });
      setShowMessageInput(false);
      setAccessMessage('');
      if (onProjectUpdate) onProjectUpdate();
    } catch (err) {
      addNotification({ type: 'error', title: 'Request Failed', message: err.message });
    } finally {
      setRequesting(false);
    }
  };
  ...
  const handlePaymentSuccess = async () => {
    ...
    addNotification({ type: 'success', title: 'Payment Successful', message: 'Your project is now published and visible to funders.' });
    if (onProjectUpdate) {
      ...
      <div className="project-actions">
        {userRole === 'borrower' && (
          <>
            ...
            <button onClick={() => navigate(`/project/${project.id}`)} className="btn btn-outline">
              View Details
            </button>
          </>
        )}
        {userRole === 'funder' && project.payment_status === 'paid' && (
          <>
            ...
            {project.access_status === 'approved' && (
              <button onClick={() => navigate(`/project/${project.id}`)} className="btn btn-primary">
                View Full Details
              </button>
            )}
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    addNotification({ type: 'success', title: 'Deal Room Created', message: 'Successfully created deal room' });
                    navigate(`/project/${project.id}/deal/${response.deal_id}`);
                  } catch (err) {
                    console.error('Deal creation error:', err);
                    addNotification({ type: 'error', title: 'Failed to create deal room', message: err.message || 'Could not create deal room' });
                  }
                }}
                className="btn btn-primary"
              >
            ...
            {project.deal_id && (
              <button onClick={() => navigate(`/project/${project.id}/deal/${project.deal_id}`)} className="btn btn-primary">
                Deal Room
              </button>
            )}
          </>
        ...
        {userRole === 'funder' && showMessageInput && (
          <div className="access-request-form">
            ...
            <div className="message-actions">
              ...
              <button onClick={handleRequestAccess} disabled={requesting} className="btn btn-sm btn-primary">
